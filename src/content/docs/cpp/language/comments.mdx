...
All comments are removed from the program at <DocLink dest="/cpp/language/translation_phases">translation phase 3</DocLink> by replacing each comment with a single whitespace character.
...
Because comments <DocLink dest="/cpp/language/translation_phases">are removed</DocLink> before the preprocessor stage, a macro cannot be used to form a comment and an unterminated C-style comment doesn't spill over from an #include'd file.
