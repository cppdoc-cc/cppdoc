...
| | | |
|-|-|-|
| `/*` <span className="t-spar">comment</span> `*/` | (1) | |
| `//` <span className="t-spar">comment</span> | (2) | |
...
C++-style comments are usually used to comment single lines, however, multiple C++-style comments can be placed together to form multi-line comments. C++-style comments tell the compiler to ignore all content between `<b>//</b>` and a new line.
...
<DescList>
  <Desc>
    <div slot="item"><DocLink src="/c/comment">C documentation</DocLink> for <span>comment</span></div>
  </Desc>
</DescList>