...
All comments are removed from the program at <DocLink src="/cpp/language/translation_phases#Phase_3">translation phase 3</DocLink> by replacing each comment with a single whitespace character.
...
Because comments <DocLink src="/cpp/language/translation_phases#Phase_3">are removed</DocLink> before the preprocessor stage, a macro cannot be used to form a comment and an unterminated C-style comment doesn't spill over from an #include'd file.
...
<DescList>
  <Desc>
    <DocLink slot="item" src="/c/comment">C documentation</DocLink>
    for `comment`
  </Desc>
</DescList>