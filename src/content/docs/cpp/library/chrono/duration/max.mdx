...
<DeclDoc>
  ...
  If the representation `rep` of the duration requires some other implementation to return a maximum-length duration, <Missing>`std::chrono::duration_values`</Missing> can be specialized to return the desired value.
</DeclDoc>
